"use strict";
var __importDefault = (this && this.__importDefault) || function (mod) {
    return (mod && mod.__esModule) ? mod : { "default": mod };
};
Object.defineProperty(exports, "__esModule", { value: true });
var jsx_runtime_1 = require("react/jsx-runtime");
var react_1 = require("react");
var ShinyText_1 = __importDefault(require("../Animations/ReactBits/ShinyText"));
var spotlight_new_1 = require("../Animations/Aceternity/spotlight-new");
var gsap_1 = require("gsap");
var ScrollTrigger_1 = require("gsap/ScrollTrigger");
gsap_1.gsap.registerPlugin(ScrollTrigger_1.ScrollTrigger);
var FounderNote = function () {
    var founderNoteRef = (0, react_1.useRef)(null);
    var imageRef = (0, react_1.useRef)(null);
    var paragraphRef = (0, react_1.useRef)(null);
    var titleRef = (0, react_1.useRef)(null);
    var companyRef = (0, react_1.useRef)(null);
    var sectionRef = (0, react_1.useRef)(null);
    (0, react_1.useEffect)(function () {
        // Set all elements to initial hidden state
        var elements = [
            founderNoteRef,
            imageRef,
            paragraphRef,
            titleRef,
            companyRef,
        ];
        elements.forEach(function (ref) {
            if (ref.current) {
                gsap_1.gsap.set(ref.current, {
                    opacity: 0,
                    y: 100,
                    willChange: "transform, opacity",
                });
            }
        });
        // Special handling for image scale
        if (imageRef.current) {
            gsap_1.gsap.set(imageRef.current, {
                opacity: 0,
                y: 120,
                scale: 0.8,
                willChange: "transform, opacity",
            });
        }
        // Single ScrollTrigger for the entire section
        if (sectionRef.current) {
            ScrollTrigger_1.ScrollTrigger.create({
                trigger: sectionRef.current,
                start: "top 80%",
                end: "bottom 20%",
                toggleActions: "play reverse play reverse",
                onEnter: function () {
                    // Animate ALL elements simultaneously with more intensity
                    elements.forEach(function (ref) {
                        if (ref.current) {
                            gsap_1.gsap.to(ref.current, {
                                opacity: 1,
                                y: 0,
                                duration: 1.5,
                                ease: "power3.out",
                                onComplete: function () {
                                    gsap_1.gsap.set(ref.current, { willChange: "auto" });
                                },
                            });
                        }
                    });
                    // Special animation for image with scale and more dramatic movement
                    if (imageRef.current) {
                        gsap_1.gsap.to(imageRef.current, {
                            opacity: 1,
                            y: 0,
                            scale: 1,
                            duration: 1.5,
                            ease: "power3.out",
                            onComplete: function () {
                                gsap_1.gsap.set(imageRef.current, { willChange: "auto" });
                            },
                        });
                    }
                },
                onLeave: function () {
                    // Hide ALL elements simultaneously
                    elements.forEach(function (ref) {
                        if (ref.current) {
                            gsap_1.gsap.to(ref.current, {
                                opacity: 0,
                                y: -30,
                                duration: 0.6,
                                ease: "power2.in",
                            });
                        }
                    });
                    if (imageRef.current) {
                        gsap_1.gsap.to(imageRef.current, {
                            opacity: 0,
                            y: -30,
                            scale: 0.95,
                            duration: 0.6,
                            ease: "power2.in",
                        });
                    }
                },
                onEnterBack: function () {
                    // Animate ALL elements simultaneously on scroll back with intensity
                    elements.forEach(function (ref) {
                        if (ref.current) {
                            gsap_1.gsap.to(ref.current, {
                                opacity: 1,
                                y: 0,
                                duration: 1.5,
                                ease: "power3.out",
                                onComplete: function () {
                                    gsap_1.gsap.set(ref.current, { willChange: "auto" });
                                },
                            });
                        }
                    });
                    if (imageRef.current) {
                        gsap_1.gsap.to(imageRef.current, {
                            opacity: 1,
                            y: 0,
                            scale: 1,
                            duration: 1.5,
                            ease: "power3.out",
                            onComplete: function () {
                                gsap_1.gsap.set(imageRef.current, { willChange: "auto" });
                            },
                        });
                    }
                },
                onLeaveBack: function () {
                    // Hide ALL elements simultaneously on scroll back with more dramatic exit
                    elements.forEach(function (ref) {
                        if (ref.current) {
                            gsap_1.gsap.to(ref.current, {
                                opacity: 0,
                                y: 100,
                                duration: 0.8,
                                ease: "power2.in",
                            });
                        }
                    });
                    if (imageRef.current) {
                        gsap_1.gsap.to(imageRef.current, {
                            opacity: 0,
                            y: 120,
                            scale: 0.8,
                            duration: 0.8,
                            ease: "power2.in",
                        });
                    }
                },
            });
        }
        // Cleanup function
        return function () {
            ScrollTrigger_1.ScrollTrigger.getAll().forEach(function (trigger) { return trigger.kill(); });
        };
    }, []);
    return ((0, jsx_runtime_1.jsxs)("section", { ref: sectionRef, id: "founder-note", style: {
            backgroundColor: "#000000",
            minHeight: "100vh",
            width: "100%",
            position: "relative",
            display: "flex",
            alignItems: "center",
            justifyContent: "center",
            padding: "2rem",
        }, children: [(0, jsx_runtime_1.jsxs)("div", { style: {
                    position: "absolute",
                    top: "10%",
                    left: "50%",
                    transform: "translateX(-50%)",
                    zIndex: 1,
                    pointerEvents: "none",
                    width: "100%",
                    maxWidth: "1200px",
                }, children: [(0, jsx_runtime_1.jsx)(spotlight_new_1.Spotlight, {}), (0, jsx_runtime_1.jsx)("h2", { ref: founderNoteRef, className: "text-4xl sm:text-6xl md:text-7xl lg:text-7xl xl:text-8xl 2xl:text-9xl font-bold opacity-90", style: {
                            fontFamily: "Inter, sans-serif",
                            color: "transparent",
                            letterSpacing: "0.02em",
                            textAlign: "center",
                            whiteSpace: "normal",
                            wordBreak: "break-word",
                            hyphens: "auto",
                            WebkitTextStroke: "1px #EEE8D6",
                            // textStroke: "1px #FFFFFF",
                            lineHeight: "0.9",
                            padding: "0 1rem",
                        }, children: (0, jsx_runtime_1.jsx)(ShinyText_1.default, { text: "FOUNDER'S NOTE", disabled: false, speed: 3 }) })] }), (0, jsx_runtime_1.jsxs)("div", { className: "relative z-10 text-center", children: [(0, jsx_runtime_1.jsx)("div", { className: "mb-8 relative", children: (0, jsx_runtime_1.jsx)("img", { ref: imageRef, src: "/websitehosttemp/images/personimage.png", 
                            // src="/images/personimage.png"
                            alt: "Founder - White Eventive", className: "w-64 h-64 md:w-80 md:h-80 lg:w-96 lg:h-96 object-cover rounded-lg mx-auto", style: {
                                // filter: "brightness(1.1) contrast(1.05)",
                                // boxShadow: "0 25px 50px -12px rgba(255, 255, 255, 0.1)",
                                maskImage: "linear-gradient(to bottom, black 60%, transparent 100%)",
                                WebkitMaskImage: "linear-gradient(to bottom, black 60%, transparent 100%)",
                            } }) }), (0, jsx_runtime_1.jsxs)("div", { className: "max-w-4xl mx-auto space-y-6", children: [(0, jsx_runtime_1.jsx)("p", { ref: paragraphRef, className: "text-lg md:text-xl lg:text-2xl leading-relaxed", style: {
                                    fontFamily: "Aileron, sans-serif",
                                    color: "#C0C0C0",
                                    lineHeight: "1.7",
                                }, children: "\"Founded in 2018, White Eventive began not as a business, but as a calling \u2014 a passion for creating spaces that transform into immersive experiences. What started with building events has evolved into a pursuit of crafting stories that spark emotion, celebrate brand identity, and leave a lasting impact. Every project is delivered with precision, creativity, and heart.\"" }), (0, jsx_runtime_1.jsxs)("div", { className: "pt-6", children: [(0, jsx_runtime_1.jsx)("p", { ref: titleRef, className: "text-xl md:text-2xl font-semibold", style: {
                                            fontFamily: "Anvers, sans-serif",
                                            color: "#EEE8D6",
                                            letterSpacing: "0.05em",
                                        }, children: (0, jsx_runtime_1.jsx)(ShinyText_1.default, { text: "Founder & CEO", disabled: false, speed: 3, className: "text-lg md:text-xl lg:text-2xl leading-relaxed" }) }), (0, jsx_runtime_1.jsx)("p", { ref: companyRef, className: "text-lg md:text-xl", style: {
                                            fontFamily: "Anvers, sans-serif",
                                            color: "#A0A0A0",
                                        }, children: "White Eventive" })] })] })] })] }));
};
exports.default = FounderNote;
